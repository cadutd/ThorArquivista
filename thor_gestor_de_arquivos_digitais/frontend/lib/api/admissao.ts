...
export type StatusSessaoSubmissao = "INICIADA" | "EM_TRANSFERENCIA" | "RECEBIDA" | "EM_QUARENTENA" | "EM_VALIDACAO" | "VALIDADA" | "REJEITADA" | "NORMALIZANDO" | "NORMALIZADA" | "FINALIZADA" | "CANCELADA";
...
export type ReuniaoAdmissao = { id: string; id_processo_admissao: string; numero_reuniao: number; titulo: string; descricao?: string | null; tipo_reuniao: TipoReuniaoAdmissao; data_reuniao: string; participantes?: string | null; deliberacoes?: string | null; pendencias?: string | null; proximos_passos?: string | null; criado_em: string; atualizado_em: string };
export type AcordoAdmissao = { id: string; id_processo_admissao: string; numero_versao: number; titulo: string; descricao?: string | null; status: StatusAcordoAdmissao; data_inicio_vigencia?: string | null; data_fim_vigencia?: string | null; motivo_revisao?: string | null; regras_empacotamento?: string | null; regras_nomenclatura?: string | null; formatos_aceitos?: string | null; metadados_obrigatorios?: string | null; requisitos_fixidez?: string | null; requisitos_representacao?: string | null; politica_validacao?: string | null; politica_rejeicao?: string | null; politica_normalizacao?: string | null; politica_sigilo?: string | null; periodicidade_submissao?: string | null; observacoes?: string | null; documento_acordo?: string | null; criado_por?: string | null; atualizado_por?: string | null; criado_em: string; atualizado_em: string };
export type SessaoSubmissao = { id: string; id_processo_admissao: string; id_acordo_utilizado: string; numero_sessao: number; titulo: string; descricao?: string | null; data_inicio: string; data_fim?: string | null; canal_submissao: CanalSubmissao; tipo_suporte: TipoSuporte; status: StatusSessaoSubmissao; responsavel_envio?: string | null; responsavel_recebimento?: string | null; volume_informado?: string | null; volume_recebido?: string | null; caminho_origem?: string | null; caminho_destino_quarentena?: string | null; resultado_validacao?: string | null; observacoes?: string | null; criado_por?: string | null; atualizado_por?: string | null; criado_em: string; atualizado_em: string };
export type SessaoSubmissaoPage = { items: SessaoSubmissao[]; total: number; limit: number; offset: number };
export type SessaoStatusPayload = { status: StatusSessaoSubmissao; volume_recebido?: string | null; resultado_validacao?: string | null };
...
export function updateReuniaoAdmissao(id: string, payload: Partial<ReuniaoAdmissao>) { return apiRequest<ReuniaoAdmissao>(`/admissao/reunioes/${id}`, { method: "PUT", body: JSON.stringify(payload) }); }
export function deleteReuniaoAdmissao(id: string) { return apiRequest<void>(`/admissao/reunioes/${id}`, { method: "DELETE" }); }
...
export function updateAcordoAdmissao(id: string, payload: Partial<AcordoAdmissao>) { return apiRequest<AcordoAdmissao>(`/admissao/acordos/${id}`, { method: "PUT", body: JSON.stringify(payload) }); }
...
export function listSessoesSubmissao(processoId: string, { limit = 20, offset = 0 }: { limit?: number; offset?: number } = {}) { return apiRequest<SessaoSubmissaoPage>(`/admissao/processos/${processoId}/sessoes${queryString({ limit, offset })}`); }
export function getSessaoSubmissao(id: string) { return apiRequest<SessaoSubmissao>(`/admissao/sessoes/${id}`); }
...
export function updateSessaoSubmissao(id: string, payload: Partial<SessaoSubmissao>) { return apiRequest<SessaoSubmissao>(`/admissao/sessoes/${id}`, { method: "PUT", body: JSON.stringify(payload) }); }
export function alterarStatusSessaoSubmissao(id: string, payload: SessaoStatusPayload) { return apiRequest<SessaoSubmissao>(`/admissao/sessoes/${id}/status`, { method: "PATCH", body: JSON.stringify(payload) }); }
